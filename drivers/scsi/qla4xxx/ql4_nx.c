		...
		u64 off, void *data, int size)
{
	...
	for (i = 0; i < loop; i++) {
		...
		if (j >= MAX_CTL_CHECK) {
			if (printk_ratelimit())
				ql4_err(ha, "failed to read through agent\n");
			break;
		}
		...
		u64 off, void *data, int size)
{
	...
	for (i = 0; i < loop; i++) {
		...
		if (j >= MAX_CTL_CHECK) {
			if (printk_ratelimit())
				ql4_err(ha, "failed to write through agent\n");
			ret = -1;
			break;
		}
	}
...
qla4_8xxx_need_reset_handler(struct scsi_qla_host *ha)
{
	...
	ql4_info(ha, "3:Device state is 0x%x = %s\n", dev_state,
...
int qla4_8xxx_device_state_handler(struct scsi_qla_host *ha)
{
	...
	dev_state = qla4_8xxx_rd_32(ha, QLA82XX_CRB_DEV_STATE);
	ql4_info(ha, "1:Device state is 0x%x = %s\n", dev_state,
		dev_state < MAX_STATES ? qdev_state[dev_state] : "Unknown");
	...
	while (1) {
		...
		dev_state = qla4_8xxx_rd_32(ha, QLA82XX_CRB_DEV_STATE);
		ql4_info(ha, "2:Device state is 0x%x = %s\n", dev_state,
		    dev_state < MAX_STATES ? qdev_state[dev_state] : "Unknown");
...
qla4_8xxx_isp_reset(struct scsi_qla_host *ha)
{
	...
	if (dev_state == QLA82XX_DEV_READY) {
		ql4_info(ha, "HW State: NEED RESET\n");
		qla4_8xxx_wr_32(ha, QLA82XX_CRB_DEV_STATE,
